...
#define RELOCATIONS_ADD(compiler_ptr, ...)                                     \
  do {                                                                         \
    ...
    _internal_reloc.program_offset =                                           \
        compiler_ptr->cur_mod_compile_info.program_size;                       \
    array_add(*compiler_ptr->cur_mod_compile.relocs, _internal_reloc);         \
  } while (0)
...
static inline void insns_add(Compiler *compiler, Instruction ins) {
  array_add(*compiler->cur_mod_compile.insns, ins);
  size_t len = ins_gen(&ins, NULL);
  compiler->cur_mod_compile_info.program_size += len;
}
...
static size_t expr_string_lit_compile(Compiler *compiler,
                                      const ExprStringLiteral *expr_string) {
  size_t offset = data_section_add(
      &compiler->cur_mod_compile_info.rodata_section, NULL,
      (uint8_t *)expr_string->string, strlen(expr_string->string) + 1);
  return offset;
}
...
#define COMPILE_CTX(...)                                                       \
  (CompileContext) { __VA_ARGS__ }

static TypeTableValue *compiler_type_get(Compiler *compiler,
                                         const char *func_name,
                                         size_t scope_idx,
                                         const ModulePath *name) {
  return tables_type_get(compiler->cur_mod_compile.function_type_tables,
                         compiler->cur_mod_compile.global_type_table,
                         (char *)func_name, scope_idx, (ModulePath *)name);
}
...
static inline ExprCompileResult
expr_compile(Compiler *compiler, const Expression *expr, CompileContext ctx) {
...
static ExprCompileResult expr_call_compile(Compiler *compiler,
                                           ...
                                           CompileContext context) {
  TypeTableValue *func_val = compiler_type_get(compiler, context.function_name,
                                               0, &expr_call->function);
  ...
  for (size_t i = 0; i < array_len(expr_call->args); i++) {
    ...
    CompileContext arg_ctx = {
        .cur_variable_type = arg_type,
        .function_name = context.function_name,
        .level = context.level,
    };
    ExprCompileResult res =
        expr_compile(compiler, &arg, arg_type == NULL ? context : arg_ctx);
    compiler_arg_push(compiler, i, &res);
  }

  Ident *mangled_function_name = hashmap_value(
      &compiler->cur_mod_compile.mangled_functions, &expr_call->function);
  ...
  Type ret_type = BUILTIN_TYPES[BUILTIN_TYPE_UNIT];
...
#define CUR_FRAME(info_ptr) ((Frame *)deque_back((info_ptr)->frames))
...
static void compiler_stack_alloc_imm8(Compiler *compiler, Ident *name, u8 imm) {
  ...
  CUR_FRAME(info)->sp_offset += sizeof(u8);
  if (name != NULL && *name != NULL) {
    hashmap_insert(&CUR_FRAME(info)->symbol_table, name,
                   &STACK_OBJ(CUR_FRAME(info)->sp_offset, sizeof(u8)));
  }
  insns_add(compiler,
            ins_mov_i8_r32_disp8(REG_EBP, -CUR_FRAME(info)->sp_offset, imm));
}
...
static void compiler_stack_alloc_imm16(Compiler *compiler, Ident *name,
  ...
  CUR_FRAME(info)->sp_offset += sizeof(u16);
  hashmap_insert(&CUR_FRAME(info)->symbol_table, name,
                 &STACK_OBJ(CUR_FRAME(info)->sp_offset, sizeof(u16)));
  insns_add(compiler,
            ins_mov_i16_r32_disp8(REG_EBP, -CUR_FRAME(info)->sp_offset, imm));
}
...
static void compiler_stack_alloc_imm32(Compiler *compiler, Ident *name,
  ...
  CUR_FRAME(info)->sp_offset += sizeof(u32);
  if (name != NULL && *name != NULL) {
    hashmap_insert(&CUR_FRAME(info)->symbol_table, name,
                   &STACK_OBJ(CUR_FRAME(info)->sp_offset, sizeof(u32)));
  }
  insns_add(compiler,
            ins_mov_i32_r32_disp8(REG_EBP, -CUR_FRAME(info)->sp_offset, imm));
}
...
static void compiler_stack_alloc_imm64(Compiler *compiler, Ident *name,
  ...
  CUR_FRAME(info)->sp_offset += sizeof(u64);

  hashmap_insert(&CUR_FRAME(info)->symbol_table, name,
                 &STACK_OBJ(CUR_FRAME(info)->sp_offset, sizeof(uint32_t)));
  insns_add(compiler, ins_mov_i64_r64(REG_EAX, imm));
  insns_add(compiler, ins_mov_r64_r64_disp32(REG_EAX, REG_EBP,
                                             -CUR_FRAME(info)->sp_offset));
}
...
static void compiler_stack_alloc_reg(Compiler *compiler, Ident *name,
  ...
  CUR_FRAME(info)->sp_offset += 8;
  if (name != NULL && *name != NULL) {
    hashmap_insert(&CUR_FRAME(info)->symbol_table, name,
                   &STACK_OBJ(CUR_FRAME(info)->sp_offset, sizeof(uint64_t)));
  }
  insns_add(compiler,
            ins_mov_r64_r64_disp8(reg, REG_EBP, -CUR_FRAME(info)->sp_offset));
}
...
static void compiler_stack_alloc_expr_res(Compiler *compiler, char *name,
                                          ExprCompileResult expr_res,
                                          size_t sp_offset) {
  ModuleCompileInfo *info = &compiler->cur_mod_compile_info;

  CUR_FRAME(info)->sp_offset = sp_offset;
  switch (expr_res.kind) {
  case EXPR_COMPILE_RES_DATA_OFFSET: {
    CUR_FRAME(info)->sp_offset += sizeof(char *);
    if (name != NULL) {
      hashmap_insert(&CUR_FRAME(info)->symbol_table, &name,
                     &STACK_OBJ(CUR_FRAME(info)->sp_offset, sizeof(char *)));
    }
    RELOCATIONS_ADD(compiler, {.sec = SECTION_FROM_EXPR_RES(expr_res.kind),
                               .data_offset = expr_res.var.data_offset.offset,
                               .r_offset = 3});
  ...
  case EXPR_COMPILE_RES_IMM: {
    ...
    case 1: {
      compiler_stack_alloc_imm8(compiler, &name, expr_res.var.imm.value);
    } break;
    case 2: {
      compiler_stack_alloc_imm16(compiler, &name, expr_res.var.imm.value);
    } break;
    case 4: {
      compiler_stack_alloc_imm32(compiler, &name, expr_res.var.imm.value);
    } break;
    case 8: {
      compiler_stack_alloc_imm64(compiler, &name, expr_res.var.imm.value);
    } break;
    }
  } break;
  case EXPR_COMPILE_RES_STACK_OBJ: {
    StackObject stack_obj = expr_res.var.stack_obj;
    // Alloc as a stack variable if the value should not be inlined
    CUR_FRAME(info)->sp_offset += stack_obj.size;
    log_debug("Size: %zu", stack_obj.size);
    StackObject ret_obj;
    if (!stack_obj.inline_val) {
      insns_add(compiler,
                ins_mov_r64_disp32_r64(REG_EBP, -stack_obj.offset, REG_EAX));
      insns_add(compiler, ins_mov_r64_r64_disp32(REG_EAX, REG_EBP,
                                                 -CUR_FRAME(info)->sp_offset));
      ret_obj = STACK_OBJ(CUR_FRAME(info)->sp_offset, stack_obj.size);
    } else {
      ret_obj = stack_obj;
    }
    if (name != NULL) {
      hashmap_insert(&CUR_FRAME(info)->symbol_table, &name, &stack_obj);
    }
  } break;
  case EXPR_COMPILE_RES_REG: {
    compiler_stack_alloc_reg(compiler, &name, expr_res.var.reg.reg);
  } break;
  case EXPR_COMPILE_RES_COMPARISON: {
    TODO();
  } break;
...
static void expr_func_compile(Compiler *compiler, const ExprFunction *expr_func,
  ...
  // TODO: Acutally push new stack frame

  Frame new_frame = {0};
  hashmap_init(&new_frame.symbol_table, &HEAP_ALLOCATOR, Ident, StackObject,
               str_ptrv_hash, str_ptrv_eq, NULL);

  deque_push_back(info->frames, new_frame);
  ...
  if (uses_stack) {
    ...
    CUR_FRAME(info)->sub_stack_size_ins_idx =
        array_len(*compiler->cur_mod_compile.insns);
  ...
  if (args_len > 0) {
    for (size_t i = 0; i < args_len; i++) {
      ...
      if (arg.kind == ARG_REGULAR_ARG) {
        Register reg;
        bool valid = reg_for_arg(&reg, i);
        if (valid) {
          compiler_stack_alloc_reg(compiler, &arg.arg_name, reg);
        } else {
          break;
        }
      }
  ...
  if (uses_stack) {
    size_t stack_size = ALIGN_BY(CUR_FRAME(info)->sp_offset, 16);
    ...
    stack_fix_sub_stack_size(compiler->cur_mod_compile.insns,
                             CUR_FRAME(info)->sub_stack_size_ins_idx,
  ...
  Frame *cur_frame = deque_pop_back(info->frames);
...
static ExprCompileResult expr_bin_op_compile(Compiler *compiler,
                                             ...
                                             const Type *res_type,
                                             CompileContext ctx) {
  CompileContext res_ctx = {
      .function_name = ctx.function_name,
      .cur_variable_type = res_type,
      .level = ctx.level,
  };
  ExprCompileResult res_left =
      expr_compile_with_res(compiler, expr_bin_op->left, res_ctx, REG_EAX);
  ExprCompileResult res_right =
      expr_compile_with_res(compiler, expr_bin_op->right, res_ctx, REG_EDX);
...
static ExprCompileResult expr_compile_with_res(Compiler *compiler,
  ...
  case EXPR_INTEGER_LIT: {
    size_t type_size;
    if (ctx.cur_variable_type != NULL) {
      type_size = type_sizeof(compiler, ctx.cur_variable_type);
    } else {
  ...
  case EXPR_CALL: {
    return expr_call_compile(compiler, &expr->var.expr_call, ctx);
  } break;
  case EXPR_IDENT: {
    Ident ident = expr->var.expr_ident.ident.path[0];
    GlobalDataLocation *data_loc =
        hashmap_value(&compiler->cur_mod_compile.globals, &ident);
    if (data_loc != NULL) {
      return EXPR_COMPILE_RES(
      ...
      StackObject *stack_obj =
          hashmap_value(&CUR_FRAME(info)->symbol_table, &ident);
      if (stack_obj != NULL) {
  ...
  case EXPR_FOR: {
    ...
    if (expr_for.has_range) {
      ...
      CompileContext range_res_ctx = {
          .cur_variable_type = &BUILTIN_TYPES[BUILTIN_TYPE_I32],
          .function_name = ctx.function_name,
          .level = ctx.level,
      };

      ExprCompileResult min_res =
          expr_compile(compiler, expr_for.range.min, range_res_ctx);
      if (min_res.kind != EXPR_COMPILE_RES_IMM) {
        log_error("Only immediate integers are supported in ranges");
        exit(1);
      }
      ExprCompileResult max_res =
          expr_compile(compiler, expr_for.range.max, range_res_ctx);
      ...
      size_t for_loop_begin = info->program_size;

      for (size_t i = 0; i < array_len(stmts); i++) {
        stmt_compile(compiler, &stmts[i], ctx);
      }

      StackObject *stack_obj =
          hashmap_value(&CUR_FRAME(info)->symbol_table, &iter_var);
      insns_add(compiler,
                ins_add_i32_r64_disp32(1, REG_EBP, -stack_obj->offset));
      if (info->program_data != NULL) {
        uint8_t offset_bytes[4] =
            IMM32_PACK(info->program_size - for_loop_begin + 6);
        for (size_t i = 0; i < 4; i++) {
          info->program_data[for_loop_begin - 1 - i] = offset_bytes[i];
        }
      }
      insns_add(compiler, ins_cmp_i32_r64_disp32(max_res.var.imm.value, REG_EBP,
                                                 -stack_obj->offset));
      insns_add(compiler,
                ins_jl_disp32(for_loop_begin - info->program_size - 6));
    } else {
      size_t for_loop_begin = info->program_size;
      ...
      insns_add(compiler,
                ins_jmp_disp32(for_loop_begin - info->program_size - 5));
      log_debug("Jmp offset: %ld",
                (int64_t)for_loop_begin - info->program_size - 5);
    }
    return (ExprCompileResult){0};
  } break;
  case EXPR_ARRAY_INIT: {
    // FIXME: Type sizes cause crash
    if (ctx.level == COMPILE_LEVEL_LOCAL) {
      ...
      size_t stack_offset = CUR_FRAME(info)->sp_offset + 8;
      for (size_t i = arr_len; i > 0; i--) {
        ExprCompileResult expr_res =
            expr_compile(compiler, &expr_arr_init.items[arr_len - i], ctx);
        compiler_stack_alloc_expr_res(compiler, NULL, expr_res,
  ...
  case EXPR_STRUCT_INIT: {
    ...
    // TODO: Make expr struct init name a mod path
    ModulePath path = mod_path_root(expr_struct_init.struct_name,
                                    &compiler->compiler_arena_allocator);
    TypeTableValue *struct_val =
        compiler_type_get(compiler, ctx.function_name, 0, &path);
    if (struct_val == NULL) {
      panic("Struct %s not defined", expr_struct_init.struct_name);
    }

    TypeExprStruct ty_expr_struct =
        struct_val->expr_variant.var.expr_var_type_expr.var.type_expr_struct;

    if (ctx.level == COMPILE_LEVEL_LOCAL) {
      LabeledExpr *field;
      array_foreach(expr_struct_init.field_inits, field) {
        ExprCompileResult field_res = expr_compile(compiler, &field->expr, ctx);
        Type field_type = BUILTIN_TYPES[BUILTIN_TYPE_UNIT];
        bool found_field_type = false;
        for (size_t i = 0; i < array_len(ty_expr_struct.fields); i++) {
          TypedIdentOptValue *decl_field_type = &ty_expr_struct.fields[i];
          if (strv_eq(decl_field_type->ident, field->field)) {
            field_type = decl_field_type->type;
            found_field_type = true;
            break;
          }
        }
        if (!found_field_type) {
          panic("Invalid struct field %s", field->field);
        }
        compiler_stack_alloc_expr_res(compiler, NULL, field_res,
                                      -(CUR_FRAME(info)->sp_offset +=
                                        type_sizeof(compiler, &field_type)));
      }
    }

    StackObject stack_obj = {
        .offset = CUR_FRAME(info)->sp_offset,
        .size = 4 * 2,
        .inline_val = true,
    };

    return EXPR_COMPILE_RES(EXPR_COMPILE_RES_STACK_OBJ, 4 * 2,
                            .stack_obj = stack_obj);
  } break;
  case EXPR_IF: {
    ...
    size_t jump_ins_idx = array_len(*compiler->cur_mod_compile.insns);
    ...
    size_t jump_program_size = info->program_size;

    for (size_t i = 0; i < array_len(expr_if.block.statements); i++) {
      Statement *stmt = &expr_if.block.statements[i];
      stmt_compile(compiler, stmt, ctx);
    }

    (*compiler->cur_mod_compile.insns)[jump_ins_idx].disp[0] =
        info->program_size - jump_program_size;
    log_debug("IF block size: %zu", info->program_size - jump_program_size);
    return (ExprCompileResult){0};
  } break;
  case EXPR_ADDR_OF: {
...
static size_t expr_res_size(const ExprCompileResult *res) {
  switch (res->kind) {
  case EXPR_COMPILE_RES_DATA_OFFSET: {
    return 8;
  } break;
  case EXPR_COMPILE_RES_IMM: {
    return res->var.imm.size;
  } break;
  case EXPR_COMPILE_RES_STACK_OBJ: {
    return res->var.stack_obj.size;
  } break;
  case EXPR_COMPILE_RES_REG: {
    return res->var.reg.reg_size;
  } break;
  case EXPR_COMPILE_RES_COMPARISON: {
    TODO();
    return 0;
...
static void stmt_compile(Compiler *compiler, const Statement *stmt,
  ...
  case STMT_EXPR: {
    ...
    if (expr.kind == EXPR_CALL) {
      ...
      expr_call_compile(compiler, &expr_call, ctx);
    } else if (expr.kind == EXPR_FOR || expr.kind == EXPR_IF) {
      expr_compile(compiler, &expr, ctx);
    }
    break;
  }
  case STMT_DECL: {
    ...
    if (stmt_decl.has_value) {
      if (ctx.level == COMPILE_LEVEL_GLOBAL) {
        DataSection *data_section =
            stmt_decl.mut ? &info->data_section : &info->rodata_section;
        if (expr.kind == EXPR_FUNCTION) {
          ModulePath func_name_mod_path = mod_path_copy(
              &compiler->cur_module->path, &compiler->compiler_arena_allocator);
          array_add(func_name_mod_path.path, stmt_decl.name);
          Ident *module_func_name =
              hashmap_value(&compiler->cur_mod_compile.mangled_functions,
                            &func_name_mod_path);
          ...
          hashmap_insert(&compiler->cur_mod_compile.function_symbols,
                         &mangled_func_name, &info->program_size);
          ...
          hashmap_insert(&compiler->cur_mod_compile.globals, &stmt_decl.name,
                         &loc);
        ...
        ExprCompileResult res = expr_compile(
            ...
            stmt_decl.type.present
                ? COMPILE_CTX(.cur_variable_type = &stmt_decl.type.type,
                              .level = ctx.level,
                              .function_name = ctx.function_name)
                : ctx);
        compiler_stack_alloc_expr_res(compiler, stmt_decl.name, res,
                                      CUR_FRAME(info)->sp_offset);
        switch (res.kind) {
        case EXPR_COMPILE_RES_IMM: {
          switch (res.var.imm.size) {
        ...
        case EXPR_COMPILE_RES_DATA_OFFSET: {
          CUR_FRAME(info)->sp_offset += sizeof(char *);
          hashmap_insert(
              &CUR_FRAME(info)->symbol_table, &stmt_decl.name,
              &STACK_OBJ(CUR_FRAME(info)->sp_offset, sizeof(char *)));
        ...
        case EXPR_COMPILE_RES_STACK_OBJ: {
          ...
          hashmap_insert(&CUR_FRAME(info)->symbol_table, &stmt_decl.name,
                         &stack_obj);
        } break;
        default: {
        } break;
      ...
      if (ctx.level == COMPILE_LEVEL_LOCAL) {
        CUR_FRAME(info)->sp_offset -=
            type_sizeof(compiler, &stmt_decl.type.type);
      }
    }
  } break;
  case STMT_RETURN: {
    ...
    if (stmt_return.has_ret_val) {
      ModulePath func_name = mod_path_root(ctx.function_name, &HEAP_ALLOCATOR);
      TypeTableValue *type_table_val =
          compiler_type_get(compiler, ctx.function_name, 0, &func_name);
      ...
      if (type_table_val != NULL &&
        ...
        if (func_expr_raw.kind == EXPR_FUNCTION) {
          ...
          func_ret_type = func_expr.desc.has_ret_type
                              ? &func_expr.desc.ret_type
                              : &BUILTIN_TYPES[BUILTIN_TYPE_UNIT];
        }
      }

      ExprCompileResult res = expr_compile(
          compiler, &stmt_return.ret_val,
          COMPILE_CTX(.cur_variable_type = func_ret_type,
                      .function_name = ctx.function_name, .level = ctx.level));
      switch (res.kind) {
      case EXPR_COMPILE_RES_IMM: {
  ...
  case STMT_ASSIGN: {
    ...
    if (stmt_assign.left_expr.kind == EXPR_IDENT) {
      ...
      StackObject *stack_obj =
          hashmap_value(&CUR_FRAME(info)->symbol_table, &ident.path[0]);

      ExprCompileResult right_expr_res =
          expr_compile(compiler, &stmt_assign.right_expr, ctx);

      if (stack_obj != NULL) {
        hashmap_insert(&CUR_FRAME(info)->symbol_table, &ident.path[0],
        ...
        case EXPR_COMPILE_RES_REG: {
          if (stmt_assign.assign_kind == ASSIGN_REGULAR) {
            ...
            insns_add(compiler,
                      ins_add_r32_r32_disp8(right_expr_res.var.reg.reg, REG_EBP,
                                            -stack_obj->offset));
          }
        } break;
        case EXPR_COMPILE_RES_COMPARISON:
      ...
      ExprCompileResult res = expr_compile(compiler, expr, ctx);
...
void compiler_compile(Compiler *compiler) {
  compiler->cur_step = COMPILE_STEP_COMPILE_SRC;

  if (debug_flags.print_compile_info) {
    log_info("[COMPILER] Start compiling file %s", "?");
  }

  size_t stmts_len = array_len(compiler->cur_mod_compile.stmts);
  while (compiler->cur_mod_compile_info.stmt_index < stmts_len) {
    stmt_compile(compiler,
                 &compiler->cur_mod_compile
                      .stmts[compiler->cur_mod_compile_info.stmt_index],
                 GLOBAL_COMPILE_CONTEXT);
    compiler->cur_mod_compile_info.stmt_index++;
  }
}
